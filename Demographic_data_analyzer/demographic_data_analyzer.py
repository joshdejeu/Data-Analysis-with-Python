...
def calculate_demographic_data(print_data=True):
    ...
    percentage_df = high_income_df.groupby('native-country').size() / df.groupby('native-country').size() * 100

    highest_earning_country = percentage_df.idxmax()
    highest_earning_country_percentage = percentage_df[percentage_df.idxmax()].round(1)

    # Identify the most popular occupation for those who earn >50K in India.
    condition = df['native-country'] == 'India'
    top_IN_occupation = df[condition]['occupation'].value_counts().idxmax()
